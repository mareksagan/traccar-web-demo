...
export default function EventReportPage() {
  ...
  const handleLoad = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`/api/reports/events?deviceId=${deviceId()}&from=${encodeURIComponent(from())}&to=${encodeURIComponent(to())}`);
  ...
  const getEventTitle = (event) => {
    ...
    if (type === 'alarm') return `${t('alarmGeneral')}: ${event.attributes?.alarm || ''}`;
  ...
  return (
    <div class="h-full flex flex-col p-4">
      ...
      <div class="flex-1 overflow-auto bg-white dark:bg-gray-800 rounded-lg shadow-sm">
        <Show when={events().length > 0} fallback={<div class="p-8 text-center text-gray-500">{t('sharedNoData')}</div>}>
          <table class="w-full text-sm"><thead class="bg-gray-50 dark:bg-gray-700 sticky top-0"><tr><th class="px-4 py-3 text-left">{t('positionFixTime')}</th><th class="px-4 py-3 text-left">{t('sharedType')}</th><th class="px-4 py-3 text-left">{t('sharedGeofence')}</th><th class="px-4 py-3 text-left">{t('maintenanceMaintenance')}</th></tr></thead>
          <tbody class="divide-y"><For each={events()}>{(event) => <tr class="hover:bg-gray-50 dark:hover:bg-gray-800"><td class="px-4 py-3">{formatTime(event.eventTime)}</td><td class="px-4 py-3">{getEventTitle(event)}</td><td class="px-4 py-3">{event.geofenceId || '-'}</td><td class="px-4 py-3">{event.maintenanceId || '-'}</td></tr>}</For></tbody></table>
        </Show>
      </div>
    </div>
  );
